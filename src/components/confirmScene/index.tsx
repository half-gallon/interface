...
const ConfirmScene = () => {
  ...
  const [numberOfSendItemTest, setNumberOfSendItemTest]  = useAtom(numberOfSendItemTestAtom);
    ...
    <SceneLayout>
      ...
      <Box sx={{mt: 8}}>
        <Box>
          ...
          <Typography variant='caption'>In case of transaction over 1,000 USDC, Voice Verification is needed</Typography>
        </Box>

        <Box sx={{mt: 2,display:'flex', flexDirection: 'column', gap: 1}}>
          <VerificationButton label='Voice verification' onClick={handleClickVoiceVerification}/>
          <VerificationButton label='Conformation' onClick={handleClickConfirmation} />
        </Box>
        <Button variant='contained' fullWidth sx={{mt: 2}} onClick={handleClickSend}>Send</Button>
      </Box>
